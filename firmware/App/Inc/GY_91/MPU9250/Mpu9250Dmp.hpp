...
#include <GY_91/MPU9250/util/inv_mpu_dmp_motion_driver.h>
}

typedef int16_t inv_error_t;
...
const int8_t defaultOrientation[9] = { 1, 0, 0, 0, 1, 0, 0, 0, 1 };
...
class MPU9250_DMP {
 public:
  ...
  // setGyroFSR(uint16_t) -- Sets the full-scale range of the gyroscope
  ...
  // setAccelFSR(uint16_t) -- Sets the FSR of the accelerometer
  //
  ...
  // Output: INV_SUCCESS (0) on success, otherwise error
  inv_error_t enableInterrupt(uint8_t enable = 1);
  ...
  // Output: INV_SUCCESS (0) on success, otherwise error
  inv_error_t dmpBegin(uint16_t features = 0, uint16_t fifoRate = MAX_DMP_SAMPLE_RATE);
  ...
  // Output: INV_SUCCESS (0) on success, otherwise error
  inv_error_t dmpSetTap(uint16_t xThresh = 250, uint16_t yThresh = 250, uint16_t zThresh = 250, uint8_t taps = 1,
                        uint16_t tapTime = 100, uint16_t tapMulti = 500);
  ...
  // Output: INV_SUCCESS (0) on success, otherwise error
  inv_error_t dmpSetOrientation(const int8_t *orientationMatrix = defaultOrientation);
  ...
  //         Bit pos 1: accel
  //         Bit pos 2: mag
  int16_t selfTest(uint8_t debug = 0);
 ...
 private:
  ...
  uint16_t orientation_row_2_scale(const int8_t *row);
};
